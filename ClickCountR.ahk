...
IntegratedGUI:
    ...
    LMBLy := winH-(winH*.155)
    LMBx := winW-(winW*.205)
    LMBy := winH-(winH*0.132)
    RMBLx := winW-(winW*0.148)
    RMBLy := winH-(winH*0.15)
    RMBx := winW-(winW*0.148)
    RMBy := winH-(winH*0.132)
    MMBLx := winW-(winW*0.178)
    MMBLy := winH-(winH*0.15)
    MMBx := winW-(winW*0.178)
    MMBy := winH-(winH*0.132)
    SwapLx := winW-(winW*.033)
    SwapLy := winH-(winH*0.05)
    Swapx := winW-(winW*.033)
    Swapy := winH-(winH*.02)
    FlaskLx := winX+(winW*0.158)
    FlaskLy := winH-(winH*0.128)
    Flaskx := winX+(winW*0.179)
    Flasky := winH-(winH*0.128)
    SkillLx := winW-(winW*.257)
    SkillLy := winH-(winH*.113)
    Skillx := winW-(winW*.238)
    Skilly := winH-(winH*.113)
	BGColor = FF00FF
    Gui, GUI_Overlay:New, -Caption +LastFound +AlwaysOnTop +ToolWindow, ClickCountR
    while (PoEWindowHwnd <= 0)
	    WinGet, PoEWindowHwnd, ID, ahk_group PoEWindowGrp
